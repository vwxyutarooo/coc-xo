...
class Linter {
  constructor() {
     ...
     */
    this.connection.onRequest(new RequestType('textDocument/xo/allFixes'), this.handleAllFixesRequest);
  ...
  log(...messages) {
    ...
    this.connection.console.log(
      ...
      messages.reduce((acc, message) => {
        if (message instanceof Map) message = `Map(${JSON.stringify([...message.entries()], null, 2)})`;
  ...
  async handleInitialize(params) {
    ...
    try {
      await Promise.all(this.foldersCache.map((folder) => this.resolveXO(folder)));
    } catch {}
  ...
  async handleDidChangeConfiguration(params) {
    if (
      ...
      this.lintDocumentDebounced = debounce(this.lintDocument, params.settings.xo.debounce, { maxWait: 350 });
    }
      ...
      this.queue.push(async () => {
        try {
          ...
          if (token.isCancellationRequested) {
            return reject(new ResponseError(LSPErrorCodes.RequestCancelled, 'Request got cancelled'));
          }
  ...
  async handleDocumentsOnDidOpen(event) {
    try {
      const { folder, config = {} } = await this.getDocumentConfig(event.document);
      const { path: customUri } = config || {};
  ...
  async resolveXO(document) {
    const { folder = {}, config = {} } = await this.getDocumentConfig(document);
    const { uri: folderUri } = folder || {};
    const { path: customPath } = config || {};
  ...
  async lintDocument(document) {
    try {
      ...
      if (!isResolutionErr) this.connection.window.showErrorMessage(error?.message ? error.message : 'Unknown Error');
  ...
  async getDocumentFolder(document) {
    ...
    folder = this.foldersCache.find((workspaceFolder) => document.uri.includes(workspaceFolder.uri));
