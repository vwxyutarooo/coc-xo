...
export async function findEslint(rootPath: string): Promise<string> {
  ...
  if (platform === 'win32' && (await exists(path.join(rootPath, 'node_modules', '.bin', `${linter}.cmd`)))) {
    return path.join('.', 'node_modules', '.bin', `${linter}.cmd`);
  } else if (
    (platform === 'linux' || platform === 'darwin') &&
    (await exists(path.join(rootPath, 'node_modules', '.bin', `${linter}`)))
  ) {
    return path.join('.', 'node_modules', '.bin', `${linter}`);
  } else {
    return `${linter}`;
  }
}
